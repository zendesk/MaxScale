...
createInstance(SERVICE *service, char **options)
{
        ...
        int                 i, nshards;
        CONFIG_PARAMETER*   param;
	char		    *weightby, *shard_ids, *shard_id;
		...
		router->servers[nservers]->shard_ids = NULL;

		shard_ids = serverGetParameter(sref->server, "shard_ids");
		if ( shard_ids ) {
			nshards = 0;
			while ( (shard_id = strsep(&shard_ids, ",")) ) {
				router->servers[nservers]->shard_ids = realloc(router->servers[nservers]->shard_ids, sizeof(int) * (nshards + 2));
				router->servers[nservers]->shard_ids[nshards] = strtol(shard_id, NULL, 0);
				nshards++;
			}
			router->servers[nservers]->shard_ids[nshards] = -1;
		}
#if defined(SS_DEBUG)
...
#endif
        client_rses->rses_prot_session = session;
	...
	GWBUF*             querybuf)
{
	...
	if (rses->rses_master_ref == NULL) {
		backend_ref_t *master_ref = NULL;
		succp = select_connect_backend_servers(&master_ref,
                                               ...
                                               1,
                                               rses->rses_config.rw_slave_select_criteria,
                                               rses->rses_prot_session,
                                               inst);

		rses->rses_master_ref = master_ref;
	}
