        ...
        GWBUF* querybuf)
{
        ...
        thd = get_or_create_thd_for_parsing(&pi->pi_handle, query_str);
	...
	GWBUF* buf)
{
	...
	if (buf == NULL ||
		...
		((parsing_info_t *)buf->gwbuf_bufobj->bo_data)->pi_handle == NULL ||
		(mysql = ((THD_DATA *) ((parsing_info_t *) buf->gwbuf_bufobj->bo_data)->pi_handle)->mysql) == NULL ||
		mysql->thd == NULL || 
...
static bool create_thd_for_parsing(MYSQL *mysql)
{
    THD *thd = NULL;
